...
function getLogicSuffix(logic: LogicType, engine: EngineType): string {
  const key = engine === "A" ? logic : (engine + logic) as keyof typeof LOGIC_SUFFIX_MAP;
...
function buildParamName(param: string, logic: LogicType, group: number, engine: EngineType, direction: "Buy" | "Sell"): string {
  const suffix = getLogicSuffix(logic, engine);
  return `gInput_${group}_${suffix}_${direction}_${param}`;
}
