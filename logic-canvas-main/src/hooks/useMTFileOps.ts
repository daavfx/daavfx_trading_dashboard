...
export function useMTFileOps(platform: Platform, externalConfig?: MTConfig | null, onLoadConfig?: (config: MTConfig) => void) {
  ...
  const loadConfigOnly = useCallback(async (newConfig: MTConfig) => {
    ...
    // Call the external callback to notify parent component (fixes dashboard not updating)
    if (onLoadConfig) {
  ...
  const importSetFileLocally = useCallback(async (configToLoad?: MTConfig) => {
    try {
      ...
      if (configToLoad) {
        ...
        console.log("[SETFILE] Total engines:", importedConfig?.engines?.length);
        ...
        importedConfig?.engines?.forEach((engine, eIdx) => {
          console.log(`[SETFILE] Engine ${engine.engine_id}: ${engine.groups?.length} groups`);
          engine.groups?.forEach((group, gIdx) => {
            console.log(`[SETFILE]   Group ${group.group_number}: ${group.logics?.length} logics`);
            group.logics?.forEach((logic, lIdx) => {
              console.log(`[SETFILE]     Logic ${logic.logic_name}: enabled=${logic.enabled}, startLevel=${logic.start_level}, initialLot=${logic.initial_lot}`);
            });
          });
        });
      }
  ...
  const exportMassiveCompleteSetfile = useCallback(async () => {
    try {
      ...
      const setfileContent = exportToSetFileWithDirections(massiveConfig as any);
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      syncActiveSetToMTCommonFiles(true, stamp).catch(() => {});
    }, 650);
