...
export function useChatCommands({ config, onConfigChange, onNavigate, onClearSelection, defaultTarget }: UseChatCommandsOptions) {
  ...
  }, [config, onConfigChange, onClearSelection, settings.autoApproveTransactions]);
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    if (lower.startsWith("/fast") || lower.startsWith("#fast")) {
      const arg = trimmed.replace(/^[/#]fast/i, "").trim().toLowerCase();
    ...
    if (lower.startsWith("/export") || lower.startsWith("#export")) {
      if (!config) {
        ...
        setMessages(prev => [...prev.slice(-(MAX_CHAT_MESSAGES - 2)), userMessage, assistantMessage]);
        setInputValue("");
        setSuggestions([]);
        return;
      }
      ...
      try {
        ...
        const configToExport = config ? withUseDirectPriceGrid(config, settings) : config;
        if (isJson) {
          await invoke("export_json_file", { config: configToExport, filePath });
        } else {
    ...
    const shouldApplyDefaultTarget =
      ...
      parsed.type === "reset";
    const command =
      parsed.type !== "unknown" && shouldApplyDefaultTarget && targetIsEmpty && defaultTarget
        ? {
            ...parsed,
            target: {
              ...parsed.target,
              engines: defaultTarget.engines?.length ? defaultTarget.engines : parsed.target.engines,
              groups: defaultTarget.groups?.length ? defaultTarget.groups : parsed.target.groups,
              logics: defaultTarget.logics?.length ? defaultTarget.logics : parsed.target.logics,
            },
          }
        : parsed;
    ...
    if (command.type !== "unknown") {
      ...
      try {
        setMessages(prev => [...prev.slice(-(MAX_CHAT_MESSAGES - 1)), {
        ...
        Current Config Platform: ${(config?.platform || "MT4")}
        ...
        const currentConfigStr = config ? JSON.stringify(config, null, 2) : "{}";
        console.log("[ChatCommand] AI Context (Config Length):", currentConfigStr.length);

        const aiResponse = await invoke<string>("ask_ai", {
          userQuery: trimmed,
          currentConfig: currentConfigStr
        });
        console.log("[ChatCommand] AI Response received:", aiResponse.substring(0, 50) + "...");
    ...
    setMessages(prev => [...prev.slice(-(MAX_CHAT_MESSAGES - 1)), assistantMessage]);
  }, [config, onConfigChange, onNavigate, defaultTarget, settings.autoApproveTransactions, updateSettingPersisted]);
