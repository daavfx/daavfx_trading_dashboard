...
export function ChatPanel({
  ...
  useEffect(() => {
    if (externalCommand) {
      ...
      // Optional: Auto-send if desired, but user might want to edit first.
      // For a smoother "gadget" feel, we could auto-send.
      // Let's just set input for now to let user confirm, or we could add a flag to auto-execute.
      // If the user wants instant action, we can call sendMessage(externalCommand).
      // Let's assume the user wants to preview the command first as per "Ryiuk understands context" tip.
    }
  }, [externalCommand, setInputValue]);
