...
pub fn export_massive_v19_setfile(
    ...
    lines.push(format!("; Total Inputs: {}", 69300));
    lines.push(String::new());
    ...
    // Export all logic-direction combinations
    let logic_names = ["POWER", "REPOWER", "SCALP", "STOPPER", "STO", "SCA", "RPO"];
    let directions = ["Buy", "Sell"];

    for engine in &config.engines {
        for group_num in 1..=15 {
            for (logic_idx, logic_name) in logic_names.iter().enumerate() {
                let group_num_u8 = group_num as u8;
                // Find the logic config for this group
                let logic_config = engine.groups.iter()
                    .find(|g| g.group_number == group_num_u8)
                    .and_then(|g| g.logics.iter().find(|l| l.logic_name == *logic_name));

                if let Some(logic) = logic_config {
                    let v19_suffix = get_v19_suffix(&engine.engine_id, logic_name);

                    for direction in &directions {
                        let dir_suffix = direction;

                        // Base parameters
                        lines.push(format!(
                            "gInput_{}_{}_{}_InitialLot={:.2}",
                            group_num, v19_suffix, dir_suffix, logic.initial_lot
                        ));
                        lines.push(format!(
                            "gInput_{}_{}_{}_Mult={:.2}",
                            group_num, v19_suffix, dir_suffix, logic.multiplier
                        ));
                        lines.push(format!(
                            "gInput_{}_{}_{}_Grid={:.1}",
                            group_num, v19_suffix, dir_suffix, logic.grid
                        ));

                        // Trail settings
                        let trail_method = match logic.trail_method.as_str() {
                            "Trail_Points" => 1,
                            "Trail_AVG_Percent" => 2,
                            "Trail_Profit_Percent" => 3,
                            _ => 0,
                        };
                        lines.push(format!(
                            "gInput_{}_{}_{}_Trail={}",
                            group_num, v19_suffix, dir_suffix, trail_method
                        ));
                        lines.push(format!(
                            "gInput_{}_{}_{}_TrailValue={:.1}",
                            group_num, v19_suffix, dir_suffix, logic.trail_value
                        ));
                        ...
                        lines.push(format!(
                            "gInput_{}_{}_{}_Start={}",
                            group_num, v19_suffix, dir_suffix, if enabled { 1 } else { 0 }
                        ));
                    }
                }
            }
...
#[allow(dead_code)]
const V19_FIELDS_PER_LOGIC: usize = 88;
#[allow(dead_code)]
const V19_TOTAL_LOGIC_DIRECTIONS: usize = V19_MAX_GROUPS * V19_MAX_ENGINES * V19_MAX_LOGICS * V19_MAX_DIRECTIONS; // 630
#[allow(dead_code)]
const V19_TOTAL_LOGIC_INPUTS: usize = V19_TOTAL_LOGIC_DIRECTIONS * V19_FIELDS_PER_LOGIC; // 55,440
#[allow(dead_code)]
const V19_TOTAL_INPUTS: usize = V19_TOTAL_LOGIC_INPUTS + 50; // ~55,500
