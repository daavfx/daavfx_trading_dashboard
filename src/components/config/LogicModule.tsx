...
export function LogicModule({
  ...
  const initialFields = getRealFields();
  ...
  useEffect(() => {
    ...
    initialFields.forEach((f) => (values[f.id] = f.value));
    setFieldValues(values);
  }, [logicConfig, groups, name, initialFields]);
  ...
  let displayFields = initialFields.map((f) => ({
    ...f,
