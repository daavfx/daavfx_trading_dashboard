...
export function DefensiveWrapper({
  ...
  useEffect(() => {
    if (renderCount.current > maxRenders) {
      console.error(`[DefensiveWrapper] ${componentName} exceeded render threshold: ${renderCount.current}`);
      onRenderThreshold?.();
  ...
  const currentProps = JSON.stringify(children);
  if (currentProps === lastProps.current) {
    consecutiveIdenticalRenders.current++;
    if (consecutiveIdenticalRenders.current > 10) {
      console.warn(`[DefensiveWrapper] ${componentName} rendering with identical props ${consecutiveIdenticalRenders.current} times`);
    }
  } else {
...
export class DefensiveErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error('[DefensiveErrorBoundary] Component error:', error);
    console.error('[DefensiveErrorBoundary] Error info:', errorInfo);
...
export function useDefensiveState<T>(
  ...
  const defensiveSetState = React.useCallback((value: T | ((prev: T) => T)) => {
    ...
    if (updateCount.current > 100) {
      console.warn('[useDefensiveState] Excessive state updates detected');
    ...
    setState(prev => {
      ...
      if (validator && !validator(newValue)) {
        console.error('[useDefensiveState] State validation failed');
        return prev;
      }
