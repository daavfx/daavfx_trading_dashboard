...
export class CommandExecutor {
  ...
  // MEMORY LEAK FIX: Limit history sizes to prevent unbounded memory growth
  private readonly MAX_PLAN_HISTORY = 50;
  private readonly MAX_REDO_STACK = 20;
  ...
  // MEMORY LEAK FIX: Helper methods to limit array sizes
  private addToPlanHistory(plan: TransactionPlan) {
    this.planHistory.push(plan);
    if (this.planHistory.length > this.MAX_PLAN_HISTORY) {
      this.planHistory = this.planHistory.slice(-this.MAX_PLAN_HISTORY);
    }
  }

  private addToRedoStack(plan: TransactionPlan) {
    this.redoStack.push(plan);
    if (this.redoStack.length > this.MAX_REDO_STACK) {
      this.redoStack = this.redoStack.slice(-this.MAX_REDO_STACK);
    }
  }
