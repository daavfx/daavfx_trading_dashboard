...
fn parse_semantic_command(input: &str) -> Option<SemanticOps> {
    ...
    // make it aggressive
    if Regex::new(r"(?:make\s*(?:it\s*)?|go\s*)(aggressive|risky)")
        .ok()?
        .is_match(&lower)
    {
        return Some(SemanticOps {
    ...
    // make it safe
    if Regex::new(r"(?:make\s*(?:it\s*)?|go\s*|play\s*(?:it\s*)?)(conservative|safe|safer)")
        .ok()?
        .is_match(&lower)
    {
        return Some(SemanticOps {
...
fn extract_target(input: &str) -> CommandTarget {
    ...
    // Extract engines
    if let Some(caps) = Regex::new(r"engine\s*([abc])")
        .ok()
    ...
    // Extract groups
    if let Some(caps) = Regex::new(r"groups?\s*(\d+)\s*-\s*(\d+)")
        .ok()
        .and_then(|r| r.captures(&lower))
    {
    ...
    } else if let Some(caps) = Regex::new(r"group\s*(\d+)")
        .ok()
        .and_then(|r| r.captures(&lower))
    {
    ...
    for (alias, logic) in logic_map {
        if Regex::new(&format!(r"\b{}\b", alias))
            .ok()
...
fn extract_params(input: &str) -> HashMap<String, Value> {
    ...
    if let Some(caps) = Regex::new(r"(?:to|=|:)\s*([\d.]+)")
        .ok()
        .and_then(|r| r.captures(&lower))
    {
        ...
        "query" => {
            if cmd.target.field.is_some() {
                ...
                CommandResult {
                    ...
                    preview: Some(json!({"mode": "snapshot", "target": cmd.target})),
                }
            }
        }
        "set" => {
        ...
        "semantic" => {
            if let Some(ref semantic) = cmd.semantic {
                CommandResult {
                    ...
                    preview: Some(
                        json!({"description": semantic.description, "operations": semantic.operations, "target": cmd.target}),
                    ),
                }
            } else {
                CommandResult {
...
fn main() {
    ...
    if args.batch {
        ...
        if args.json {
            println!(
                "{}",
                serde_json::to_string_pretty(&results).unwrap_or("[]".into())
            );
        } else {
