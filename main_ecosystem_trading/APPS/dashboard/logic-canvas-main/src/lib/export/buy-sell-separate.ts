    ...
    engines: config.engines.map(engine => ({
      ...
      groups: engine.groups.map(group => ({
        ...
        logics: group.logics.map(logic => ({
          ...
          close_partial_mode: "PartialMode_Mid" as const,
