// MT4/MT5 Bridge - File-based communication with trading platforms
// ACCURATE MAPPING - only handles fields that actually exist in MT4
...
use crate::mql_rust_compiler::{MQLRustCompiler, ValidationReport, PrecompilationResult, CompilationError};
...
fn sanitize_and_validate_path(path: &PathBuf) -> Result<PathBuf, String> {
    ...
    if existing_prefix.exists() {
        let mut final_path = existing_prefix.canonicalize()
    ...
    for component in absolute_path.components() {
        match component {
            ...
            std::path::Component::ParentDir => {
                ...
                if !normalized.pop() {
                    return Err(format!("Path traversal attempt detected (root escape): {:?}", path));
                }
            }
...
fn validate_path_within_base(path: &PathBuf, base_dir: &PathBuf) -> Result<PathBuf, String> {
    ...
    if sanitized.starts_with(&base_dir_normalized) {
        ...
        Err(format!("Path must be within the base directory: {:?}", base_dir_normalized))
    }
}
...
fn atomic_write(path: &PathBuf, content: &str) -> Result<(), String> {
    ...
    let tmp_extension = format!("{}.tmp", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_nanos());
...
fn obfuscate_string(input: &str) -> String {
    ...
    if input.starts_with("ENC:") { return input.to_string(); }
...
pub struct LogicConfig {
    ...
    #[serde(default)] pub trail_step_2: Option<f64>,
    #[serde(default)] pub trail_step_method_2: Option<String>,
    #[serde(default)] pub trail_step_cycle_2: Option<i32>,
    #[serde(default)] pub trail_step_balance_2: Option<f64>,
    #[serde(default)] pub trail_step_mode_2: Option<String>,

    #[serde(default)] pub trail_step_3: Option<f64>,
    #[serde(default)] pub trail_step_method_3: Option<String>,
    #[serde(default)] pub trail_step_cycle_3: Option<i32>,
    #[serde(default)] pub trail_step_balance_3: Option<f64>,
    #[serde(default)] pub trail_step_mode_3: Option<String>,

    #[serde(default)] pub trail_step_4: Option<f64>,
    #[serde(default)] pub trail_step_method_4: Option<String>,
    #[serde(default)] pub trail_step_cycle_4: Option<i32>,
    #[serde(default)] pub trail_step_balance_4: Option<f64>,
    #[serde(default)] pub trail_step_mode_4: Option<String>,

    #[serde(default)] pub trail_step_5: Option<f64>,
    #[serde(default)] pub trail_step_method_5: Option<String>,
    #[serde(default)] pub trail_step_cycle_5: Option<i32>,
    #[serde(default)] pub trail_step_balance_5: Option<f64>,
    #[serde(default)] pub trail_step_mode_5: Option<String>,

    #[serde(default)] pub trail_step_6: Option<f64>,
    #[serde(default)] pub trail_step_method_6: Option<String>,
    #[serde(default)] pub trail_step_cycle_6: Option<i32>,
    #[serde(default)] pub trail_step_balance_6: Option<f64>,
    #[serde(default)] pub trail_step_mode_6: Option<String>,

    #[serde(default)] pub trail_step_7: Option<f64>,
    #[serde(default)] pub trail_step_method_7: Option<String>,
    #[serde(default)] pub trail_step_cycle_7: Option<i32>,
...
fn default_half_scale() -> f64 { 50.0 }
...
impl MTBridgeState {
    ...
    pub fn initialize_compiler(&self) -> Result<(), String> {
        ...
        let mt4_str = mt4_path.as_ref().map(|p| p.to_string_lossy().to_string()).unwrap_or_default();
        let mt5_str = mt5_path.as_ref().map(|p| p.to_string_lossy().to_string()).unwrap_or_default();
...
pub async fn load_mt_config(
    ...
    let config: MTConfig = serde_json::from_str(&json_str)
...
pub async fn get_default_mt4_path() -> Result<String, String> {
    // First check if MT4 terminal exists
    ...
    // Also check common files location as fallback
...
pub async fn get_default_mt5_path() -> Result<String, String> {
    // First check if MT5 terminal exists
    let terminal_paths = vec![
...
pub fn export_set_file(
    ...
    lines.push(format!("gInput_UseDirectPriceGrid={}", if config.general.use_direct_price_grid { 1 } else { 0 }));
    ...
    let is_absolute = file_path.contains(":") || file_path.starts_with("\\\\") || file_path.starts_with("/");
    if is_absolute {
        ...
        lines.push(format!("gInput_ConfigFileIsCommon={}", if config.general.config_file_is_common { 1 } else { 0 }));
    }
    ...
    lines.push(format!("gInput_RequireLicense={}", if config.general.require_license { 1 } else { 0 }));
    lines.push(format!("gInput_LicenseCheckInterval={}", config.general.license_check_interval));
    lines.push(String::new());
    
    // Compounding
    lines.push("; === COMPOUNDING ===".to_string());
    lines.push(format!("gInput_Input_Compounding={}", if config.general.compounding_enabled { 1 } else { 0 }));
    ...
    lines.push(format!("gInput_UseSpreadFilter={}", if config.general.risk_management.spread_filter_enabled { 1 } else { 0 }));
    lines.push(format!("gInput_MaxSpreadPoints={:.1}", config.general.risk_management.max_spread_points));
    lines.push(format!("gInput_UseEquityStop={}", if config.general.risk_management.equity_stop_enabled { 1 } else { 0 }));
    lines.push(format!("gInput_EquityStopValue={:.1}", config.general.risk_management.equity_stop_value));
    lines.push(format!("gInput_UseDrawdownStop={}", if config.general.risk_management.drawdown_stop_enabled { 1 } else { 0 }));
    lines.push(format!("gInput_MaxDrawdownPercent={:.1}", config.general.risk_management.max_drawdown_percent));
    if let Some(risk_action) = config.general.risk_management.risk_action.as_deref() {
        lines.push(format!("gInput_RiskAction={}", trigger_action_to_int(risk_action)));
    }
    lines.push(String::new());
    ...
    lines.push(format!("gInput_EnableNewsFilter={}", if config.general.news_filter.enabled { 1 } else { 0 }));
    lines.push(format!("gInput_NewsFilterEnabled={}", if config.general.news_filter.enabled { 1 } else { 0 }));
    lines.push(format!("gInput_NewsAPIKey={}", config.general.news_filter.api_key));
    lines.push(format!("gInput_NewsAPIURL={}", config.general.news_filter.api_url));
    lines.push(format!("gInput_NewsFilterCountries={}", config.general.news_filter.countries));
    lines.push(format!("gInput_NewsImpactLevel={}", config.general.news_filter.impact_level));
    lines.push(format!("gInput_MinutesBeforeNews={}", config.general.news_filter.minutes_before));
    lines.push(format!("gInput_MinutesAfterNews={}", config.general.news_filter.minutes_after));
    lines.push(format!("gInput_NewsAction={}", trigger_action_to_int(&config.general.news_filter.action)));
    ...
    lines.push(format!("gInput_NewsFilterOverridesSession={}", if config.general.time_filters.priority_settings.news_filter_overrides_session { 1 } else { 0 }));
    lines.push(format!("gInput_SessionFilterOverridesNews={}", if config.general.time_filters.priority_settings.session_filter_overrides_news { 1 } else { 0 }));
    lines.push(format!("gInput_NewsOverridesSession={}", if config.general.time_filters.priority_settings.news_filter_overrides_session { 1 } else { 0 }));
    lines.push(format!("gInput_SessionOverridesNews={}", if config.general.time_filters.priority_settings.session_filter_overrides_news { 1 } else { 0 }));
    let any_session_enabled = config.general.time_filters.sessions.iter().any(|s| s.enabled);
    lines.push(format!("gInput_SessionFilterEnabled={}", if any_session_enabled { 1 } else { 0 }));
    ...
    for (i, session) in config.general.time_filters.sessions.iter().enumerate() {
        ...
        lines.push(format!("gInput_Session{}StartHour={}", session_num, session.start_hour));
        lines.push(format!("gInput_Session{}StartMinute={}", session_num, session.start_minute));
        lines.push(format!("gInput_Session{}EndHour={}", session_num, session.end_hour));
        lines.push(format!("gInput_Session{}EndMinute={}", session_num, session.end_minute));
        lines.push(format!("gInput_Session{}Action={}", session_num, trigger_action_to_int(&session.action)));
    }
    lines.push(String::new());
        ...
        for group in &engine.groups {
            ...
            if group.group_number > 1 {
                if let Some(gps) = group.group_power_start {
                    ...
                    lines.push(format!("gInput_GroupPowerStart_P{}={}", group.group_number, gps));
                    lines.push(format!("gInput_GroupPowerStart_BP{}={}", group.group_number, gps));
                    lines.push(format!("gInput_GroupPowerStart_CP{}={}", group.group_number, gps));
                }
            }
            
            // Group-level Reverse/Hedge controls (V17.04+)
            lines.push(format!("gInput_Group{}_ReverseMode={}", group.group_number, if group.reverse_mode { 1 } else { 0 }));
            lines.push(format!("gInput_Group{}_HedgeMode={}", group.group_number, if group.hedge_mode { 1 } else { 0 }));
            lines.push(format!("gInput_Group{}_HedgeReference={}", group.group_number, group.hedge_reference));
            lines.push(format!("gInput_Group{}_EntryDelayBars={}", group.group_number, group.entry_delay_bars));
            
            for logic in &group.logics {
                let suffix = get_logic_suffix(&engine.engine_id, group.group_number, &logic.logic_name);
                let short = get_logic_short(&engine.engine_id, &logic.logic_name);
                
                lines.push(format!("gInput_Start_{}={}", suffix, if logic.enabled { 1 } else { 0 }));
                ...
                lines.push(format!("gInput_G{}_{}_StrategyType={}", group.group_number, short, logic.strategy_type));
                lines.push(format!("gInput_G{}_{}_TradingMode={}", group.group_number, short, logic.trading_mode));
                ...
                lines.push(format!("gInput_AllowBuy_{}={}", suffix, if allow_buy { 1 } else { 0 }));
                lines.push(format!("gInput_AllowSell_{}={}", suffix, if allow_sell { 1 } else { 0 }));

                if include_optimization_hints {
                    let (f, start, step, stop) = get_optimization_values("initial_lot", logic.initial_lot);
                ...
                if include_optimization_hints {
                    let (f, start, step, stop) = get_optimization_values("multiplier", logic.multiplier);
                ...
                lines.push(format!("gInput_TrailValue_{}={:.1}", suffix, logic.trail_value));
                ...
                lines.push(format!("gInput_Trail_Start_{}={:.1}", suffix, logic.trail_start));
                ...
                lines.push(format!("gInput_TrailStep_{}={:.1}", suffix, logic.trail_step));
                ...
                lines.push(format!("gInput_TrailStepMethod_{}={}", suffix, encode_trail_step_method(&logic.trail_step_method)));
                
                // Trail Step Advanced (V17.04+)
                lines.push(format!("gInput_TrailStepMode_{}={}", suffix, encode_trail_step_mode(&logic.trail_step_mode)));
                lines.push(format!("gInput_TrailStepCycle_{}={}", suffix, logic.trail_step_cycle));
                lines.push(format!("gInput_TrailStepBalance_{}={:.2}", suffix, logic.trail_step_balance));
                
                // Trail Step Extended (Levels 2-7)
                // Level 2
                if let Some(v) = logic.trail_step_2 { lines.push(format!("gInput_TrailStep2_{}={:.1}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_method_2 { lines.push(format!("gInput_TrailStepMethod2_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_cycle_2 { lines.push(format!("gInput_TrailStepCycle2_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_balance_2 { lines.push(format!("gInput_TrailStepBalance2_{}={:.2}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_mode_2 { lines.push(format!("gInput_TrailStepMode2_{}={}", suffix, v)); }

                // Level 3
                if let Some(v) = logic.trail_step_3 { lines.push(format!("gInput_TrailStep3_{}={:.1}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_method_3 { lines.push(format!("gInput_TrailStepMethod3_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_cycle_3 { lines.push(format!("gInput_TrailStepCycle3_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_balance_3 { lines.push(format!("gInput_TrailStepBalance3_{}={:.2}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_mode_3 { lines.push(format!("gInput_TrailStepMode3_{}={}", suffix, v)); }

                // Level 4
                if let Some(v) = logic.trail_step_4 { lines.push(format!("gInput_TrailStep4_{}={:.1}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_method_4 { lines.push(format!("gInput_TrailStepMethod4_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_cycle_4 { lines.push(format!("gInput_TrailStepCycle4_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_balance_4 { lines.push(format!("gInput_TrailStepBalance4_{}={:.2}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_mode_4 { lines.push(format!("gInput_TrailStepMode4_{}={}", suffix, v)); }

                // Level 5
                if let Some(v) = logic.trail_step_5 { lines.push(format!("gInput_TrailStep5_{}={:.1}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_method_5 { lines.push(format!("gInput_TrailStepMethod5_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_cycle_5 { lines.push(format!("gInput_TrailStepCycle5_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_balance_5 { lines.push(format!("gInput_TrailStepBalance5_{}={:.2}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_mode_5 { lines.push(format!("gInput_TrailStepMode5_{}={}", suffix, v)); }

                // Level 6
                if let Some(v) = logic.trail_step_6 { lines.push(format!("gInput_TrailStep6_{}={:.1}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_method_6 { lines.push(format!("gInput_TrailStepMethod6_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_cycle_6 { lines.push(format!("gInput_TrailStepCycle6_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_balance_6 { lines.push(format!("gInput_TrailStepBalance6_{}={:.2}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_mode_6 { lines.push(format!("gInput_TrailStepMode6_{}={}", suffix, v)); }

                // Level 7
                if let Some(v) = logic.trail_step_7 { lines.push(format!("gInput_TrailStep7_{}={:.1}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_method_7 { lines.push(format!("gInput_TrailStepMethod7_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_cycle_7 { lines.push(format!("gInput_TrailStepCycle7_{}={}", suffix, v)); }
                if let Some(v) = logic.trail_step_balance_7 { lines.push(format!("gInput_TrailStepBalance7_{}={:.2}", suffix, v)); }
                if let Some(ref v) = logic.trail_step_mode_7 { lines.push(format!("gInput_TrailStepMode7_{}={}", suffix, v)); }

                
                // Logic specific
                if group.group_number == 1 {
                    if let Some(sl) = logic.start_level {
                        if let Some(start_key) = get_logic_start_key(&engine.engine_id, &logic.logic_name) {
                            lines.push(format!("gInput_{}={}", start_key, sl));
                        }
                    }
                }
                
                lines.push(format!("gInput_CloseTargets_{}={}", suffix, logic.close_targets));
                ...
                if group.group_number == 1 {
                    ...
                    lines.push(format!("gInput_CloseTargets_{}={}", logic_key, close_targets));
                }
                if group.group_number == 1 {
                    let logic_key = get_logic_global_key(&engine.engine_id, &logic.logic_name);
                    lines.push(format!("gInput_{}_OrderCountReference={}", logic_key, logic.order_count_reference));
                }
                lines.push(format!("gInput_MaxPowerOrders_{}={}", suffix, engine.max_power_orders));
                lines.push(format!("gInput_OrderCountReference_{}={}", suffix, logic.order_count_reference));
                lines.push(format!("gInput_ResetLotOnRestart_{}={}", suffix, if logic.reset_lot_on_restart { 1 } else { 0 }));
                
                // TPSL - use group-aware naming
                lines.push(format!("gInput_G{}_UseTP_{}={}", group.group_number, short, if logic.use_tp { 1 } else { 0 }));
                lines.push(format!("gInput_G{}_TP_Mode_{}={}", group.group_number, short, logic.tp_mode));
                lines.push(format!("gInput_G{}_TP_Value_{}={:.1}", group.group_number, short, logic.tp_value));
                if include_optimization_hints && logic.use_tp {
                     let (f, start, step, stop) = get_optimization_values("tp_value", logic.tp_value);
                     lines.push(format!("gInput_G{}_TP_Value_{},F={}", group.group_number, short, f));
                     lines.push(format!("gInput_G{}_TP_Value_{},1={:.1}", group.group_number, short, start));
                     lines.push(format!("gInput_G{}_TP_Value_{},2={:.1}", group.group_number, short, step));
                     lines.push(format!("gInput_G{}_TP_Value_{},3={:.1}", group.group_number, short, stop));
                }

                lines.push(format!("gInput_G{}_UseSL_{}={}", group.group_number, short, if logic.use_sl { 1 } else { 0 }));
                lines.push(format!("gInput_G{}_SL_Mode_{}={}", group.group_number, short, logic.sl_mode));
                lines.push(format!("gInput_G{}_SL_Value_{}={:.1}", group.group_number, short, logic.sl_value));
                if include_optimization_hints && logic.use_sl {
                     let (f, start, step, stop) = get_optimization_values("sl_value", logic.sl_value);
                     lines.push(format!("gInput_G{}_SL_Value_{},F={}", group.group_number, short, f));
                     lines.push(format!("gInput_G{}_SL_Value_{},1={:.1}", group.group_number, short, start));
                     lines.push(format!("gInput_G{}_SL_Value_{},2={:.1}", group.group_number, short, step));
                     lines.push(format!("gInput_G{}_SL_Value_{},3={:.1}", group.group_number, short, stop));
                }

                // Reverse/Hedge per-logic (V17.04+ full structure)
                lines.push(format!("gInput_G{}_{}_ReverseEnabled={}", group.group_number, short, if logic.reverse_enabled { 1 } else { 0 }));
                lines.push(format!("gInput_G{}_{}_HedgeEnabled={}", group.group_number, short, if logic.hedge_enabled { 1 } else { 0 }));
                lines.push(format!("gInput_G{}_Scale_{}_Reverse={:.1}", group.group_number, short, logic.reverse_scale));
                lines.push(format!("gInput_G{}_Scale_{}_Hedge={:.1}", group.group_number, short, logic.hedge_scale));
                lines.push(format!("gInput_G{}_{}_ReverseReference={}", group.group_number, short, logic.reverse_reference));
                lines.push(format!("gInput_G{}_{}_HedgeReference={}", group.group_number, short, logic.hedge_reference));
                
                // Close Partial - use correct variable names
                lines.push(format!("gInput_ClosePartial_{}={}", suffix, if logic.close_partial { 1 } else { 0 }));
                ...
                // Close Partial Extended (Levels 2-4)
                // Level 2
                if let Some(v) = logic.close_partial_2 { lines.push(format!("gInput_ClosePartial2_{}={}", suffix, if v { 1 } else { 0 })); }
...
pub fn export_set_file_to_mt_common_files(
    ...
    export_set_file(config, path_str.clone(), platform, include_optimization_hints, None, None, None)?;
    Ok(path_str)
}
...
pub fn _export_vault_file_to_mt_common_files(
    ...
    let set_content = if source_file_path.ends_with(".json") {
        ...
        let mut lines = vec![
            format!("; DAAVFX Configuration - Generated {}", 
...
pub fn get_active_set_status() -> Result<ActiveSetStatus, String> {
    ...
    for line in content.lines() {
        ...
        if let Some(pos) = line.find('=') {
            ...
            keys_total += 1;
            if key.contains("ginput_start_") || key.ends_with("_start") || key.contains("_enabled_") || key.ends_with("_enabled") {
                keys_start += 1;
            }
        }
    }
...
pub async fn import_set_file(
    ...
    // Check file size (limit to 5MB to prevent DoS)
    let metadata = fs::metadata(&sanitized_path)
    ...
    let bytes = fs::read(&sanitized_path)
        .map_err(|e| format!("Failed to read .set file: {}", e))?;
    ...
    for line in content.lines() {
        ...
        if let Some(pos) = line.find('=') {
            ...
            if key.chars().any(|c| !c.is_alphanumeric() && c != '_' && c != '.') {
                continue;
            }
            ...
            let value = if raw_value.contains("||") {
                ...
                raw_value.split(",F=").next().unwrap_or("").trim().to_string()
            } else {
                raw_value.to_string()
            };
    ...
    println!("[SETFILE] Rust: Parsed {} lines, {} key-value pairs", line_count, key_count);
    
    // Debug: Show some sample keys
    let sample_keys: Vec<&String> = values.keys().take(10).collect();
    println!("[SETFILE] Rust: Sample keys: {:?}", sample_keys);
...
pub async fn import_json_file(
    ...
    let mut config: MTConfig = serde_json::from_str(&json_str)
...
pub fn _export_set_file_with_validation(
    ...
    // Perform the export
    export_set_file(config.clone(), file_path.clone(), platform.clone(), include_optimization_hints, None, None, None)?;
...
fn get_vault_path() -> PathBuf {
    ...
    for i in 0..15 {
        ...
        // Legacy nesting
        let candidate_nested = current.join("daavfx_trading_ecosystem_6.0").join("Vault_Presets");
...
pub async fn list_vault_files(vault_path_override: Option<String>) -> Result<VaultListing, String> {
    ...
    let process_dir = |dir: PathBuf, category: Option<String>| -> Result<Vec<VaultFile>, std::io::Error> {
                        ...
                        if let Some(ext) = path.extension() {
                            ...
                            if ext_str == "set" || ext_str == "json" {
                                ...
                                if let Ok(content) = fs::read_to_string(&path) {
                                    if ext_str == "json" {
                                         if let Ok(wrapper) = serde_json::from_str::<VaultJson>(&content) {
                                             tags = wrapper.metadata.tags;
                                             comments = wrapper.metadata.comments;
                                             magic_number = Some(wrapper.config.general.magic_number);
                                         } else if let Ok(config) = serde_json::from_str::<MTConfig>(&content) {
                                        ...
                                        for line in content.lines().take(200) {
                                            if line.starts_with("; Tags: ") {
                                                tags = Some(line.trim_start_matches("; Tags: ").split(',').map(|s| s.trim().to_string()).collect());
                                            } else if line.starts_with("; Comments: ") {
                                                comments = Some(line.trim_start_matches("; Comments: ").to_string());
                                            } else if line.contains("gInput_MagicNumber=") || line.contains("MagicNumber=") {
                                                let parts: Vec<&str> = line.split('=').collect();
                                                if parts.len() >= 2 {
                                                    let val_str = parts[1].split(';').next().unwrap_or("").trim();
...
#[tauri::command]
pub async fn get_vault_size(vault_path_override: Option<String>) -> Result<VaultSizeResult, String> {
    let vault_path = resolve_vault_path(vault_path_override)?;
    if !vault_path.exists() {
...
fn find_latest_terminal_log(root: &PathBuf) -> Option<PathBuf> {
    ...
    for entry in entries.flatten() {
        ...
        for log_entry in logs.flatten() {
            ...
            if p.extension().and_then(|e| e.to_str()).unwrap_or("").to_lowercase() != "log" {
                continue;
            }
...
fn read_tail_lines(path: &PathBuf, max_lines: usize) -> Result<Vec<String>, String> {
    ...
    let size = f.metadata().map_err(|e| format!("Failed to stat log: {}", e))?.len();
...
pub async fn read_recent_terminal_log(lines: u32) -> Result<TerminalLogTail, String> {
    ...
    let latest = find_latest_terminal_log(&root).ok_or_else(|| "No terminal log files found".to_string())?;
...
#[tauri::command]
pub async fn _export_vault_file(
    filename: String,
    target_path: String,
) -> Result<(), String> {
    ...
    if target_path.to_lowercase().ends_with(".json") {
        ...
        // Default to .set
        export_set_file(config, target_path, "Export".to_string(), false, None, None, None)?;
    }
...
pub async fn save_to_vault(
    ...
    if let Some(cat) = category {
        if !cat.is_empty() {
            let safe_cat = cat.replace(|c: char| !c.is_alphanumeric() && c != '-' && c != '_' && c != ' ', "_");
    ...
    if file_format.to_lowercase() == "json" {
        ...
        let file_path = validated_file_path;
        // Reuse export logic
        export_set_file(config_safe, file_path.to_string_lossy().to_string(), "Vault".to_string(), false, None, tags, comments)?;
    }
...
#[tauri::command]
pub async fn _delete_from_vault(filename: String, vault_path_override: Option<String>) -> Result<(), String> {
    let vault_root = resolve_vault_path(vault_path_override)?;
    let file_path_buf = vault_root.join(filename);
    let validated_file_path = validate_path_within_base(&file_path_buf, &vault_root)?;
    
    if validated_file_path.exists() {
        fs::remove_file(validated_file_path).map_err(|e| format!("Failed to delete file: {}", e))?;
    }
...
fn get_bool(values: &std::collections::HashMap<String, String>, key: &str) -> bool {
    values.get(key).map(|v| v == "1" || v.to_lowercase() == "true").unwrap_or(false)
}
...
fn get_string(values: &std::collections::HashMap<String, String>, key: &str, default: &str) -> String {
    values.get(key).cloned().unwrap_or_else(|| default.to_string())
}

fn build_config_from_values(values: &std::collections::HashMap<String, String>) -> Result<MTConfig, String> {
    ...
    for i in 1..=7 {
        sessions.push(SessionConfig {
            ...
            action: get_string(values, &format!("gInput_Session{}Action", i), "Action_Default"),
            auto_restart: get_bool(values, &format!("gInput_Session{}AutoRestart", i)),
            restart_mode: get_string(values, &format!("gInput_Session{}RestartMode", i), "Restart_Default"),
    ...
    let general = GeneralConfig {
        ...
        license_server_url: get_string(values, "gInput_LicenseServerURL", "https://license.daavfx.com"),
        ...
        restart_policy_non_power: get_string(values, "gInput_RestartPolicy_NonPower", "Restart_Default"),
        ...
        magic_number_buy: get_i32(values, "gInput_MagicNumberPowerBuy", 777),
        magic_number_sell: get_i32(values, "gInput_MagicNumberPowerSell", 8988),
        max_slippage_points: get_f64(values, "gInput_MaxSlippagePoints", 30.0),
    ...
    println!("[SETFILE] Rust: Built {} engines from setfile", engines.len());
    for engine in &engines {
        println!("[SETFILE] Rust:   Engine {}: {} groups", engine.engine_id, engine.groups.len());
    }
...
/// Format: gInput_{ParamName}_{LogicCode}{Group}[_Sell]
/// Examples:
///   gInput_Initial_loT_P1 -> (Initial_loT, A, 1, Power, Buy, P1)
///   gInput_Initial_loT_P1_Sell -> (Initial_loT, A, 1, Power, Sell, P1_Sell)
///   gInput_Initial_loT_BP1 -> (Initial_loT, B, 1, Power, Buy, BP1)
///   gInput_Initial_loT_BP1_Sell -> (Initial_loT, B, 1, Power, Sell, BP1_Sell)
fn parse_parameter_name(name: &str) -> Option<ParsedParameter> {
    ...
    // Find the last underscore to check for _Sell or _Buy suffix
    let parts: Vec<&str> = rest.rsplitn(2, '_').collect();
    
    let (param_and_logic, direction) = if parts.len() == 2 {
        let last_part = parts[0];
        let rest_part = parts[1];
        
        if last_part == "Sell" {
            (rest_part, "Sell")
        } else if last_part == "Buy" {
            (rest_part, "Buy")
        } else {
            // No direction suffix, default to Buy
            (rest, "Buy")
        }
    } else {
        (rest, "Buy")
    ...
    // Now parse param_and_logic to separate parameter name from logic code
    // Find the last underscore before the logic code
    let last_underscore = param_and_logic.rfind('_')?;
    let param_name = &param_and_logic[..last_underscore];
    let logic_code = &param_and_logic[last_underscore + 1..];
    
    // Parse the logic code: P1, BP1, R1, BR1, ST1, BST1, etc.
...
fn build_engines_from_values(values: &std::collections::HashMap<String, String>) -> Result<Vec<EngineConfig>, String> {
    use std::collections::HashMap;
    
    // Count total V4 parameters before parsing
    let total_params = values.len();
    let v4_params: Vec<&String> = values.keys()
        .filter(|k| k.starts_with("gInput_"))
        .collect();
    
    println!("[SETFILE] Rust: Total V4 parameters: {} / {}", v4_params.len(), total_params);
    
    // Structure: Engine -> Group -> Logic -> Direction -> Params
    let mut engine_data: HashMap<String, HashMap<u8, HashMap<String, HashMap<String, HashMap<String, String>>>>> = HashMap::new();
    ...
    for (key, value) in values {
        if let Some(parsed) = parse_parameter_name(key) {
            let engine_entry = engine_data.entry(parsed.engine.clone()).or_insert_with(HashMap::new);
            let group_entry = engine_entry.entry(parsed.group).or_insert_with(HashMap::new);
            let logic_entry = group_entry.entry(parsed.logic.clone()).or_insert_with(HashMap::new);
            let direction_entry = logic_entry.entry(parsed.direction.clone()).or_insert_with(HashMap::new);
    ...
    println!("[SETFILE] Rust: Successfully parsed {} parameters", parsed_count);
    if !failed_params.is_empty() {
        println!("[SETFILE] Rust: Failed to parse {} parameters (showing first 10)", failed_params.len());
        for param in failed_params.iter().take(10) {
            println!("[SETFILE] Rust:   - {}", param);
        }
    }
    ...
    for (engine, groups) in &engine_data {
        println!("[SETFILE] Rust:   Engine {}: {} groups", engine, groups.len());
        let mut total_logics = 0;
        let mut total_directions = 0;
        for (group_num, logics) in groups {
            println!("[SETFILE] Rust:     Group {}: {} logics", group_num, logics.len());
            for (logic, directions) in logics {
                total_logics += 1;
                println!("[SETFILE] Rust:       {}: {} directions", logic, directions.len());
                total_directions += directions.len();
            }
        }
        println!("[SETFILE] Rust:     Total: {} logics, {} directions", total_logics, total_directions);
    }
    ...
    for engine_id in &engine_ids {
        ...
        if let Some(group_data) = engine_data.get(*engine_id) {
            // Sort groups by number
            let mut group_numbers: Vec<u8> = group_data.keys().cloned().collect();
            group_numbers.sort();
            
            for group_num in group_numbers {
                if let Some(logic_data) = group_data.get(&group_num) {
        ...
        // If no groups found, create default Group 1
        if groups.is_empty() {
            groups.push(create_default_group(1));
        }
    ...
    println!("[SETFILE] Rust: Final config - {} engines, {} groups, {} logics, {} directions", 
             engines.len(), total_groups, total_logics, total_directions);
...
fn build_group_config(
    engine_id: &str,
    group_num: u8,
    logic_data: &std::collections::HashMap<String, std::collections::HashMap<String, std::collections::HashMap<String, String>>>,
    ...
    for logic_name in &logic_order {
        if let Some(direction_data) = logic_data.get(*logic_name) {
    ...
    let hedge_reference = get_string(values, &format!("gInput_Group{}_HedgeReference", group_num), "Logic_None");
    let entry_delay_bars = get_i32(values, &format!("gInput_Group{}_EntryDelayBars", group_num), 0);
...
fn build_logic_config(
    ...
    let get_param = |param: &str, default: &str| -> String {
        // First try to find in Buy direction (which has all base parameters)
    ...
    let get_param_f64 = |param: &str, default: f64| -> f64 {
        get_param(param, &default.to_string()).parse().unwrap_or(default)
    };
    
    let get_param_i32 = |param: &str, default: i32| -> i32 {
        get_param(param, &default.to_string()).parse().unwrap_or(default)
    };
    
    let get_param_bool = |param: &str| -> bool {
        let v = get_param(param, "0");
        v == "1" || v.to_lowercase() == "true"
    };
    
    // Parse base parameters
    let initial_lot = get_param_f64("Initial_loT", 0.02);
    let initial_lot_b = buy_params.and_then(|m| m.get("Initial_loT_B")).and_then(|v| v.parse().ok());
    let initial_lot_s = sell_params.and_then(|m| m.get("Initial_loT_S")).and_then(|v| v.parse().ok());
    
    let multiplier = get_param_f64("Mult", 1.2);
    let multiplier_b = buy_params.and_then(|m| m.get("Mult_B")).and_then(|v| v.parse().ok());
    let multiplier_s = sell_params.and_then(|m| m.get("Mult_S")).and_then(|v| v.parse().ok());
    
    let grid = get_param_f64("Grid", 300.0);
    let grid_b = buy_params.and_then(|m| m.get("Grid_B")).and_then(|v| v.parse().ok());
    let grid_s = sell_params.and_then(|m| m.get("Grid_S")).and_then(|v| v.parse().ok());
    
    let trail_method = get_param("Trail", "0");
    let trail_value = get_param_f64("TrailValue", 3000.0);
    let trail_value_b = buy_params.and_then(|m| m.get("TrailValue_B")).and_then(|v| v.parse().ok());
    let trail_value_s = sell_params.and_then(|m| m.get("TrailValue_S")).and_then(|v| v.parse().ok());
    
    let trail_start = get_param_f64("Trail_Start", 1.0);
    let trail_start_b = buy_params.and_then(|m| m.get("Trail_Start_B")).and_then(|v| v.parse().ok());
    let trail_start_s = sell_params.and_then(|m| m.get("Trail_Start_S")).and_then(|v| v.parse().ok());
    
    let trail_step = get_param_f64("TrailStep", 1500.0);
    let trail_step_b = buy_params.and_then(|m| m.get("TrailStep_B")).and_then(|v| v.parse().ok());
    let trail_step_s = sell_params.and_then(|m| m.get("TrailStep_S")).and_then(|v| v.parse().ok());
    
    let trail_step_method = get_param("TrailStepMethod", "0");
    
    // Parse logic-specific parameters
    let start_level = if logic_name != "Power" {
        let key = format!("Start{}", logic_name);
        Some(get_param_i32(&key, 4))
    } else {
        None
    };
    
    let last_lot = if logic_name != "Power" {
        let key = format!("LastLot{}", logic_name);
        Some(get_param_f64(&key, 0.12))
    } else {
        Some(get_param_f64("LastLotPower", 0.63))
    };
    
    let close_targets = get_param("CloseTargets", &default_close_targets(engine_id, logic_name));
    let order_count_reference = get_param("OrderCountReference", "Logic_None");
    let reset_lot_on_restart = get_param_bool("ResetLotOnRestart");
    
    // Parse mode selectors
    let strategy_type = get_param("StrategyType", "Trail");
    let trading_mode = get_param("TradingMode", "Trending");
    let allow_buy = get_param_bool("AllowBuy");
    let allow_sell = get_param_bool("AllowSell");
    
    // Parse TPSL parameters
    let use_tp = get_param_bool(&format!("G{}_UseTP_{}", group_num, short_logic));
    let tp_mode = get_param(&format!("G{}_TP_Mode_{}", group_num, short_logic), "TP_Pips");
    let tp_value = get_param_f64(&format!("G{}_TP_Value_{}", group_num, short_logic), 100.0);
    let use_sl = get_param_bool(&format!("G{}_UseSL_{}", group_num, short_logic));
    let sl_mode = get_param(&format!("G{}_SL_Mode_{}", group_num, short_logic), "SL_Pips");
    let sl_value = get_param_f64(&format!("G{}_SL_Value_{}", group_num, short_logic), 100.0);
    
    // Parse reverse/hedge parameters
    let reverse_enabled = get_param_bool(&format!("G{}_{}_ReverseEnabled", group_num, short_logic));
    let hedge_enabled = get_param_bool(&format!("G{}_{}_HedgeEnabled", group_num, short_logic));
    let reverse_scale = get_param_f64(&format!("G{}_Scale_{}_Reverse", group_num, short_logic), 100.0);
    let hedge_scale = get_param_f64(&format!("G{}_Scale_{}_Hedge", group_num, short_logic), 50.0);
    let reverse_reference = get_param(&format!("G{}_{}_ReverseReference", group_num, short_logic), "Logic_None");
    let hedge_reference = get_param(&format!("G{}_{}_HedgeReference", group_num, short_logic), "Logic_None");
    
    // Parse trail step advanced parameters
    let trail_step_mode = get_param("TrailStepMode", "TrailStepMode_Auto");
    let trail_step_cycle = get_param_i32("TrailStepCycle", 1);
    let trail_step_balance = get_param_f64("TrailStepBalance", 0.0);
    
    // Parse close partial parameters
    let close_partial = get_param_bool("ClosePartial");
    ...
    Ok(LogicConfig {
        ...
        grid_b,
        grid_s,
        trail_method: if trail_method == "0" { "Trail".to_string() } else { trail_method },
        trail_value,
        trail_value_b,
        trail_value_s,
        trail_start,
...
#[tauri::command]
pub async fn initialize_mql_compiler(
    state: State<'_, MTBridgeState>,
) -> Result<(), String> {
...
pub async fn run_precompilation_pipeline(
    ...
    if let Some(ref mut compiler) = *compiler_guard {
        compiler.run_precompilation_pipeline()
...
pub async fn start_mql_file_watching(
    ...
    if let Some(ref mut compiler) = *compiler_guard {
        ...
        compiler.start_file_watching(callback)
...
pub async fn get_mql_compiler_status(
    ...
    if let Some(ref compiler) = *compiler_guard {
        Ok(MQLCompilerStatus {
            initialized: true,
            mt4_files_found: compiler.project.main_files.iter()
                .filter(|f| f.to_string_lossy().contains("mq4")).count(),
            mt5_files_found: compiler.project.main_files.iter()
                .filter(|f| f.to_string_lossy().contains("mq5")).count(),
...
mod tests {
    ...
    fn test_export_set_file_includes_new_magic_number_fields() {
        ...
        let result = export_set_file(config, file_path.clone(), "MT4".to_string(), false, None, None, None);
        assert!(result.is_ok(), "Export should succeed: {:?}", result);
        
        let file_content = std::fs::read_to_string(&file_path).expect("Failed to read exported file");
        assert!(file_content.contains("gInput_MagicNumberPowerBuy=123"), "File missing buy magic number");
        assert!(file_content.contains("gInput_MagicNumberPowerSell=456"), "File missing sell magic number");
        assert!(file_content.contains("gInput_UseDirectPriceGrid=1"), "File missing UseDirectPriceGrid");
    ...
    fn test_build_config_from_values_includes_new_magic_number_fields() {
        ...
        values.insert("gInput_MagicNumberPowerBuy".to_string(), "123".to_string());
        values.insert("gInput_MagicNumberPowerSell".to_string(), "456".to_string());
        ...
        values.insert("gInput_LicenseServerURL".to_string(), "http://test".to_string());
    ...
    fn test_mql_compiler_validation() {
        ...
        if report.total_errors > 0 {
            ...
            for error in &report.errors {
                println!("  [{:?}] {}:{} - {}", error.severity, error.file, error.line, error.message);
        ...
        assert!(report.critical_errors == 0, "Critical errors found in MQL validation");
    }
...
fn extract_broker_name(terminal_path: &PathBuf) -> String {
    ...
    if let Some(name) = terminal_path.file_name() {
        ...
        if name_str.contains("terminal64") {
            ...
            return name_str.replace("terminal64.exe - ", "").replace("terminal64 ", "");
        }
...
pub async fn get_mt4_settings() -> Result<MT4Settings, String> {
    ...
    for entry in fs::read_dir(&terminal_root).map_err(|e| format!("Failed to read terminal folder: {}", e))? {
    ...
    match latest_terminal {
        Some((_, terminal_path)) => {
            ...
            let profiles_path = terminal_path.join("profiles")
...
pub async fn configure_mt4_path(path: String) -> Result<MT4Settings, String> {
    ...
    let profiles_path = path_buf.join("profiles")
        .to_string_lossy().to_string();
